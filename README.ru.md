...
4. Преобразует текст в речь через **TTS** (облачный Edge TTS, локальный Piper или Supertonic 3)
...
- (Опционально) `pip install supertonic` — для Supertonic 3 (локальный, высокое качество, ~305 МБ)
...
| 6 | Настраиваете комментарии: частота (каждые N предложений), роль комментатора, свой промпт, **и выбираете TTS движок** (Edge TTS облачный, Piper локальный или Supertonic 3 локальный) |
...
- Доступные голоса: 5 женских (F1-F5) + 5 мужских (M1-M5)

| Язык | Основной голос (текст) | Голос комментатора |
|------|----------------------|-------------------|
| 🇷🇺 Русский | **F1 — Анна** (женский) | **M1 — Порфирий** (мужской) |
| 🇬🇧 Английский | **F1** (женский) | **M1** (мужской) |

**Установка:** `pip install supertonic` — модель скачается автоматически при первом запуске.
...
│   │                          # TTS (абстрактная база + Edge + Piper + Supertonic 3), сборка аудио,
...
Можно изменить: язык интерфейса, язык книги, AI-провайдер, TTS движок (edge/piper/supertonic), голоса TTS и скорость, длительность пауз, частоту комментариев, папку вывода.
...
→ Проверьте версию Python (3.11+). В редких случаях может потребоваться `pip install --upgrade pip` перед установкой.
